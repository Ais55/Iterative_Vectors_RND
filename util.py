...
def load_tokenizer_and_model(args) -> tuple[AutoTokenizer, AutoModelForCausalLM]:
    ...
    name_or_path = str(args.model)  # ✅ Ensure it's always a string
    ...
    if args.device_map is None:  # load small model directly
        ...
        print(
            ...
            f"with gpu {os.environ.get('CUDA_VISIBLE_DEVICES', 'unknown')}"
        )
...
def load_dataset_splits(dataset, task=None, val_pct=20):
    """
    Load train, validation, and test splits from Hugging Face datasets.
    Falls back to splitting train into train/val if validation split is missing.
    """
    if task and task != "None":
        ds = load_dataset(dataset, task, trust_remote_code=True)
    else:
        ds = load_dataset(dataset, trust_remote_code=True)

    if "train" not in ds or "test" not in ds:
        raise ValueError(f"{dataset}/{task} must contain train and test splits!")

    if "validation" in ds:
        train_set = ds["train"]
        val_set = ds["validation"]
    else:
        split_point = 100 - val_pct
        train_set = ds["train"].select(range(int(len(ds["train"]) * split_point / 100)))
        val_set = ds["train"].select(range(int(len(ds["train"]) * split_point / 100), len(ds["train"])))
    ...
    train_set, val_set, test_set = map(trec_rename, (train_set, val_set, test_set))
