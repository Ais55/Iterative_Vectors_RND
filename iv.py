...
def extract_iv(full_token, ans_mask, shift=True, mask_interm_ans=False):
    """Extract iterative vectors from the model."""
    ...
    with TraceDict(model, layers=args.target_layers) as t:
        logits = model(input_ids=full_token["input_ids"], attention_mask=attn_mask)["logits"]
...
def apply_iv(full_token, io_mask, ans_mask, iv, strength=1):
    """Apply iterative vectors to the model at target layers."""
    ...
    with TraceDict(model, layers=args.target_layers, edit_output=editor, retain_output=False):
...
def extract_and_apply_iv(
    ...
    """Extract and apply iterative vectors simultaneously."""
    shift_ans_mask = ans_mask[:, 1:] if shift else ans_mask
    attn_mask = full_token["attention_mask"]
    ...
    if iv is not None:
        iv_to_add = {
            l: torch.cat(
                (v.gather(0, support_label.unsqueeze(1).expand(-1, v.size(1))), v[-1:]),
            )
            for l, v in iv.items()
        }
...
# ====================================================
#          RBF KERNEL-BASED ATTENTION AGGREGATION
# ====================================================

def attention_aggregate(iv_list, sigma=0.2):
    """
    RBF (Gaussian) kernel-based attention aggregation over episodes.
    
    Args:
        iv_list: tensor of shape [episodes, classes, hidden_dim]
        sigma: RBF kernel bandwidth (smaller -> sharper focus on representative episodes)
        
    Returns:
        agg: tensor of shape [classes, hidden_dim]
    """
    eps, C, D = iv_list.shape
    device = iv_list.device

    # Reference per-class: mean across episodes
    ref = iv_list.mean(dim=0, keepdim=True)  # [1, C, D]

    # Cosine similarity between each episode and reference
    iv_norm = F.normalize(iv_list, p=2, dim=-1)
    ref_norm = F.normalize(ref, p=2, dim=-1)
    sim = (iv_norm * ref_norm).sum(dim=-1)  # [eps, C], cosine similarity in [-1,1]

    # RBF weights
    weights = torch.exp(-(1 - sim) / (2 * sigma ** 2))  # sharper weights for similar episodes
    weights = weights / weights.sum(dim=0, keepdim=True)  # normalize per class

    # Weighted sum over episodes
    agg = torch.einsum("ic,icd->cd", weights, iv_list)  # [C, D]
...
if not args.run_clean:
    for ext_ep in tqdm(range(args.episodes), desc="Extraction"):
        ...
        if (args.ext_batch and ext_ep % args.ext_batch == 0) or ext_ep == args.episodes - 1:
            ivs = {
                l: attention_aggregate(torch.stack(x), sigma=0.2)
                for l, x in extracted.items()
            }
...
for _ in tqdm(range(args.eval_episodes), desc="Eval"):
    support, query = target_set.random_episode(rng, args.shot, args.run_clean or args.shot > 1)
...
if args.run_clean:
    ...
    with open(args.clean_result_fn, "a") as f:
        f.write(
            f"task={args.vtask} shot={args.shot} eval_episodes={args.eval_episodes} model={args.model_name} "
        )
    ...
    with open(save_fn, "a") as f:
        f.write(
            f"task={args.vtask} model={args.model_name} shot={args.shot} "
            f"ext_shot={args.ext_shot} strength={args.strength} ext_strength={args.ext_strength} "
            f"ext_batch={args.ext_batch} episodes={args.episodes} eval_episodes={args.eval_episodes} seed={args.seed} "
        )
        f.write(f"{' '.join(f'{k}={v * 100:.2f}' for k, v in f1.items())}\n")
